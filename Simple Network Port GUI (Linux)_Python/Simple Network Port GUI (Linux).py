...
def addPortRule(port, protocol, action): #protocol can be tcp or udp, action can be DROP or ACCEPT
    command='iptables -A INPUT -p '+protocol+' --destination-port '+port+' -j '+action
    print(command)
...
def remPortRule(port, protocol, action): #Same parameters as addPortRule, only change is the delete keyword in the command to delete
    command='iptables -D INPUT -p '+protocol+' --destination-port '+port+' -j '+action
    run([command], shell=True)

def listPorts(): #returns the list of ports
    command= 'iptables -S'
    output=run([command], shell=True, stdout=PIPE, stderr=PIPE, universal_newlines=True)
...
def addRemRule():
    ...
    if portAction == "DROP" or portAction == "ACCEPT":
        pass
    else:
        lbl_result["text"] = "Please input 'ACCEPT' or\n 'DROP' for Action"
        ruleCheck = False
...
# Set up the window
window = tk.Tk()
window.title("Simple Network Port GUI (Linux)")
...
lbl_action = tk.Label(text="Action (ACCEPT/DROP)",bg='#C0D4D8')
...
#ent_password.grid(row=3, column=2, padx = 10, sticky="w")
...
#lbl_title2.grid(row=1, column=2, sticky="e")
